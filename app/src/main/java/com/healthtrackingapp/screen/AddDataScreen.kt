...
fun AddDataScreen(navController: NavHostController, viewModel: HealthViewModel,editId: Int = -1) {
   ...
   /* Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Add Health Data", color = Color.White) },
            ...
            Button(
                onClick = {
                    ...
                    if (heartRate.isNotEmpty()) {
                        viewModel.addMetric(HealthMetric(type = "Heart Rate", value = heartRate.toFloat(), timestamp = currentTime))
                    }
                    if (steps.isNotEmpty()) {
                        viewModel.addMetric(HealthMetric(type = "Steps", value = steps.toFloat(), timestamp = currentTime))
                    }
                    if (waterIntake.isNotEmpty()) {
                        viewModel.addMetric(HealthMetric(type = "Water Intake", value = waterIntake.toFloat(), timestamp = currentTime))
                    }
                    navController.popBackStack()
                ...
                Text("Submit", style = MaterialTheme.typography.titleMedium, color = Color.White)
            }
        }
    ) { padding ->
        Column(
